                        ...
                        <div class="sm:col-span-2">
                            ...
                            <div class="flex gap-4 items-center flex-wrap mb-5">
                                @foreach ($technologies as $technology)
                                    <input name="technologies[]" class="text-sm font-medium" type="checkbox"
                                        ...
                                        @checked(in_array($technology->id, old('technologies', $project->technology->pluck('id')->all())))>
                            ...
                            <div class="w-full">
                                ...
                                <input type="number" name="stars" id="stars"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                    value="{{ $project->stars }}" placeholder="5" required="" min="0"
                                    max="5">
                            </div>
                            <div>
                                <label for="category"
                                ...
                                <select id="type_id" name="type_id"
                                    ...
                                    @foreach ($types as $type)
                                        <option @selected(optional($project->type)->id == $type->id) value="{{ $type->id }}">
                                            {{ $type->name }}
                                        </option>
                                    @endforeach
                                </select>
